...
v2.0   26 sept 25  - add wifi features => does NOT work with SDCard :(
...
const int delayBeforeSleep = 200;  // number of 500ms before sleep
const char* ssid = "photoWIFI";
const char* password = "";

// wifi
//-------------------
#include "WiFi.h"
WiFiServer server(80);         // set web server on port #80
bool wifiPhoto = false;
...
// start AP wifi & web server
//---------------------------
  WiFi.mode(WIFI_AP);           // drop station mode
  WiFi.softAP(ssid, password);
  server.begin();
  String mess = "wifi: ";
  mess += String(ssid);
  tft.drawString(mess, 160, 160);
  mess = "http://";
  mess += WiFi.softAPIP().toString();
  tft.drawString(mess, 160, 180);
...
//----------------
  ...
  if(client.connected()) {
    ...
    if(menuDelay < 0) {
      ...
      client.print("<!DOCTYPE html><html lang=fr-FR>");
      client.print("<HEAD><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">");
      client.print("<link rel=\"icon\" href=\"data:,\">");
      client.print("<body><h1>Appareil photo chez Fifi</h1>");
      client.print("<center><p><h3>Apparail photo en mode menu.<br>Attendre un peu et rafraichir la page.</h3></p>\n");
      client.print("</body></center></html>");                   
      //client.stop();
      //wifiPhoto = false;
    }
  }
